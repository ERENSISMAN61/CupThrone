...
{
    // How far the viewer needs to move before we update terrain
    ...
    private void Update()
    {
        ...
        // Only if viewer has moved far enough do we update visible chunks
    ...
    public class TerrainChunk
    {
        ...
        public TerrainChunk(Vector3 coord, int size, LODInfo[] detailLevels, Transform parent, Material material)
        {
        ...
        public void UpdateTerrainChunk()
        {
            if (mapDataReceived)
            {
                ...
                if (visible)
                {
                    ...
                    // Only if player is close enough for the terrain to be rendered at its highest resolution
                    if (lodIndex == 0)
                    {
    ...
    {
        public int lod;
        // Defines how far the viewer needs to be from it to switch to next LOD
